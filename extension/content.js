(() => {
  ...
  chrome.runtime.onMessage.addListener((m, sender, reply) => {
    ...
    if (m.type === 'select') {
      ...
      let start, rect, done=false, dragging=false, placed=false, zoom=m.zoom!==false, cursor={x:innerWidth/2,y:innerHeight/2};
      ...
      const key=e=>{
        if(!['Escape','Enter','ArrowLeft','ArrowRight','ArrowUp','ArrowDown','z','Z'].includes(e.key))return;
        e.preventDefault();e.stopImmediatePropagation();
        if(e.key==='Escape'){finish({cancelled:true});return;}
        if(e.key.toLowerCase()==='z'){zoom=!zoom;paint();return;}
        if(e.key==='Enter'){
          if(m.kind==='click'){finish({point:cursor,geometry:geometry()});return;}
          if(rect&&rect.width>0&&rect.height>0){finish({rect,geometry:geometry()});return;}
          start={...cursor};return;
        }
        const dx=e.key==='ArrowLeft'?-1:e.key==='ArrowRight'?1:0,dy=e.key==='ArrowUp'?-1:e.key==='ArrowDown'?1:0;
        if(placed&&rect){
          rect.x=Math.max(0,Math.min(innerWidth-rect.width,rect.x+dx));
          rect.y=Math.max(0,Math.min(innerHeight-rect.height,rect.y+dy));
          drawRect();
        }else{cursor={x:Math.max(0,Math.min(innerWidth,cursor.x+dx)),y:Math.max(0,Math.min(innerHeight,cursor.y+dy))};paint();}
      };
      ...
      function paint(){
        ...
        if(start&&!placed&&m.kind==='capture'){
          rect={x:Math.min(x,start.x),y:Math.min(y,start.y),width:Math.abs(x-start.x),height:Math.abs(y-start.y)};
          drawRect();
        }
      }
      snapshot.onload=paint;
      function move(e){cursor=position(e);paint();}
      overlay.onpointerdown=e=>{if(e.button!==0)return;e.preventDefault();placed=false;dragging=true;start=position(e);overlay.setPointerCapture(e.pointerId);move(e);};
      overlay.onpointermove=move;
      overlay.onpointerup=e=>{if(e.button===0&&dragging){e.preventDefault();move(e);dragging=false;placed=!!rect&&rect.width>0&&rect.height>0;}};
      overlay.onclick=e=>{
        e.preventDefault();e.stopPropagation();
        if(m.kind==='click')finish({point:position(e),geometry:geometry()});
      };
      paint();
      return true;
    }
